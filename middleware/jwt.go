...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		fmt.Println()
		fmt.Println("-----------中间件JWT鉴权-----------------, token: ", token)
		fmt.Println()
		if token == "" {
		...
		if err != nil {
			if err == TokenExpired {
				response.FailWithDetailed(gin.H{"reload": true}, "Token授权已过期", c)
				c.Abort()
				return
			}
		...
		if claims.ExpiresAt-time.Now().Unix() < claims.BufferTime {
			...
			if global.GSD_CONFIG.System.UseMultipoint {
				_, err = jwtService.GetRedisJWT(newClaims.UserID)
				if err != nil {
					global.GSD_LOG.Error("get redis jwt failed", zap.Any("err", err), utils.GetRequestID(c))
				}
				// 无论如何都要记录当前的活跃状态
				_ = jwtService.SetRedisJWT(newToken, newClaims.UserID)
			}
		}
		if userInfo, err := jwtService.GetRedisUserInfo(claims.UUID.String()); err != nil {
			//载入上下文
			response.FailWithDetailed(gin.H{"reload": true}, "Redis授权已过期", c)
			c.Abort()
		} else {
